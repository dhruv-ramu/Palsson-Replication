#!/usr/bin/env python3
"""
Custom Metabolic Model for Experimental Conditions

This script builds a custom metabolic model specifically designed to match
the experimental 13C-MFA data for glucose, acetate, and lactose minimal media.
"""
...
class CustomMetabolicModel:
    """
    Custom metabolic model built specifically for experimental conditions.
    """
    ...
    def build_custom_model(self):
        """Build a custom metabolic model based on experimental data."""
        print("=" * 70)
        print("BUILDING CUSTOM METABOLIC MODEL")
        print("=" * 70)
        
        # Create a new model
        self.model = cobra.Model("Custom_E_coli_Model")
        ...
        metabolites = {
            ...
            'nh4_e': cobra.Metabolite('nh4_e', name='Ammonium', compartment='e'),
            ...
            'fdp_c': cobra.Metabolite('fdp_c', name='Fructose-1,6-bisphosphate', compartment='c'),
            ...
            'succoa_c': cobra.Metabolite('succoa_c', name='Succinyl-CoA', compartment='c'),
            ...
            'ru5p__D_c': cobra.Metabolite('ru5p__D_c', name='D-Ribulose-5-phosphate', compartment='c'),
            'glyc3p_c': cobra.Metabolite('glyc3p_c', name='Glyceraldehyde-3-phosphate', compartment='c'),
            'dhap_c': cobra.Metabolite('dhap_c', name='Dihydroxyacetone-phosphate', compartment='c'),
            '3pg_c': cobra.Metabolite('3pg_c', name='3-Phospho-D-glycerate', compartment='c'),
            '2pg_c': cobra.Metabolite('2pg_c', name='2-Phospho-D-glycerate', compartment='c'),
            ...
            'pi_c': cobra.Metabolite('pi_c', name='Phosphate', compartment='c'),
            'nh4_c': cobra.Metabolite('nh4_c', name='Ammonium', compartment='c'),
        }
        ...
        self.add_pentose_phosphate_reactions()
        self.add_glyoxylate_cycle_reactions()
        self.add_lactose_metabolism_reactions()
    ...
    def add_glycolysis_reactions(self):
        ...
        reactions = [
            # Glucose uptake
            cobra.Reaction('EX_glc__D_e', name='Glucose exchange', subsystem='Exchange'),
            
            # Hexokinase
            cobra.Reaction('HEX1', name='Hexokinase', subsystem='Glycolysis'),
            
            # Phosphoglucose isomerase
            cobra.Reaction('PGI', name='Phosphoglucose isomerase', subsystem='Glycolysis'),
            
            # Phosphofructokinase
            cobra.Reaction('PFK', name='Phosphofructokinase', subsystem='Glycolysis'),
            
            # Aldolase
            cobra.Reaction('FBA', name='Fructose-bisphosphate aldolase', subsystem='Glycolysis'),
            
            # Triose phosphate isomerase
            cobra.Reaction('TPI', name='Triose-phosphate isomerase', subsystem='Glycolysis'),
            
            # Glyceraldehyde-3-phosphate dehydrogenase
            cobra.Reaction('GAPD', name='Glyceraldehyde-3-phosphate dehydrogenase', subsystem='Glycolysis'),
            
            # Phosphoglycerate kinase
            cobra.Reaction('PGK', name='Phosphoglycerate kinase', subsystem='Glycolysis'),
            
            # Phosphoglycerate mutase
            cobra.Reaction('PGM', name='Phosphoglycerate mutase', subsystem='Glycolysis'),
            
            # Enolase
            cobra.Reaction('ENO', name='Enolase', subsystem='Glycolysis'),
            
            # Pyruvate kinase
            cobra.Reaction('PYK', name='Pyruvate kinase', subsystem='Glycolysis'),
        ]
        ...
        self.model.reactions.get_by_id('PGI').add_metabolites({
            self.model.metabolites.get_by_id('g6p_c'): -1,
        ...
        self.model.reactions.get_by_id('PFK').add_metabolites({
            self.model.metabolites.get_by_id('f6p_c'): -1,
            self.model.metabolites.get_by_id('atp_c'): -1,
            self.model.metabolites.get_by_id('fdp_c'): 1,
            self.model.metabolites.get_by_id('adp_c'): 1,
            self.model.metabolites.get_by_id('h_c'): 1
        })
        
        self.model.reactions.get_by_id('FBA').add_metabolites({
            self.model.metabolites.get_by_id('fdp_c'): -1,
            self.model.metabolites.get_by_id('glyc3p_c'): 1,
            self.model.metabolites.get_by_id('dhap_c'): 1
        })
        
        self.model.reactions.get_by_id('TPI').add_metabolites({
            self.model.metabolites.get_by_id('dhap_c'): -1,
            self.model.metabolites.get_by_id('glyc3p_c'): 1
        })
        
        self.model.reactions.get_by_id('GAPD').add_metabolites({
            self.model.metabolites.get_by_id('glyc3p_c'): -1,
            self.model.metabolites.get_by_id('pi_c'): -1,
            self.model.metabolites.get_by_id('nad_c'): -1,
            self.model.metabolites.get_by_id('1,3bpg_c'): 1,
            self.model.metabolites.get_by_id('nadh_c'): 1,
            self.model.metabolites.get_by_id('h_c'): 1
        })
        
        self.model.reactions.get_by_id('PGK').add_metabolites({
            self.model.metabolites.get_by_id('1,3bpg_c'): -1,
            self.model.metabolites.get_by_id('adp_c'): -1,
            self.model.metabolites.get_by_id('3pg_c'): 1,
            self.model.metabolites.get_by_id('atp_c'): 1
        })
        
        self.model.reactions.get_by_id('PGM').add_metabolites({
            self.model.metabolites.get_by_id('3pg_c'): -1,
            self.model.metabolites.get_by_id('2pg_c'): 1
        })
        
        self.model.reactions.get_by_id('ENO').add_metabolites({
            self.model.metabolites.get_by_id('2pg_c'): -1,
    ...
    def add_tca_cycle_reactions(self):
        """Add TCA cycle reactions."""
        reactions = [
            # Pyruvate dehydrogenase
            cobra.Reaction('PDH', name='Pyruvate dehydrogenase', subsystem='TCA Cycle'),
            
            # Citrate synthase
            cobra.Reaction('CS', name='Citrate synthase', subsystem='TCA Cycle'),
            
            # Aconitase
            cobra.Reaction('ACONT', name='Aconitase', subsystem='TCA Cycle'),
            
            # Isocitrate dehydrogenase
            cobra.Reaction('ICDHyr', name='Isocitrate dehydrogenase', subsystem='TCA Cycle'),
            
            # α-Ketoglutarate dehydrogenase
            cobra.Reaction('AKGDH', name='α-Ketoglutarate dehydrogenase', subsystem='TCA Cycle'),
            
            # Succinyl-CoA synthetase
            cobra.Reaction('SUCOAS', name='Succinyl-CoA synthetase', subsystem='TCA Cycle'),
            
            # Succinate dehydrogenase
            cobra.Reaction('SUCDi', name='Succinate dehydrogenase', subsystem='TCA Cycle'),
            
            # Fumarase
            cobra.Reaction('FUM', name='Fumarase', subsystem='TCA Cycle'),
            
            # Malate dehydrogenase
            cobra.Reaction('MDH', name='Malate dehydrogenase', subsystem='TCA Cycle'),
        ]
        ...
        # Set reaction stoichiometry (simplified for brevity)
        # PDH: pyruvate + NAD+ + CoA -> acetyl-CoA + NADH + CO2
        self.model.reactions.get_by_id('PDH').add_metabolites({
            self.model.metabolites.get_by_id('pyr_c'): -1,
            self.model.metabolites.get_by_id('nad_c'): -1,
        ...
        # CS: acetyl-CoA + oxaloacetate -> citrate + CoA
        ...
        # AKGDH: α-ketoglutarate + NAD+ + CoA -> succinyl-CoA + NADH + CO2
        self.model.reactions.get_by_id('AKGDH').add_metabolites({
            self.model.metabolites.get_by_id('akg_c'): -1,
            self.model.metabolites.get_by_id('nad_c'): -1,
            self.model.metabolites.get_by_id('coa_c'): -1,
            self.model.metabolites.get_by_id('succoa_c'): 1,
            self.model.metabolites.get_by_id('nadh_c'): 1,
            self.model.metabolites.get_by_id('co2_e'): 1
        })
        
        # SUCOAS: succinyl-CoA + ADP + Pi -> succinate + ATP + CoA
        self.model.reactions.get_by_id('SUCOAS').add_metabolites({
            self.model.metabolites.get_by_id('succoa_c'): -1,
            self.model.metabolites.get_by_id('adp_c'): -1,
            self.model.metabolites.get_by_id('pi_c'): -1,
        ...
        # SUCDi: succinate + FAD -> fumarate + FADH2
    ...
    def add_pentose_phosphate_reactions(self):
        """Add pentose phosphate pathway reactions."""
        reactions = [
            # Glucose-6-phosphate dehydrogenase
            cobra.Reaction('G6PDH2r', name='Glucose-6-phosphate dehydrogenase', subsystem='Pentose Phosphate'),
            
            # 6-Phosphogluconolactonase
            cobra.Reaction('PGL', name='6-Phosphogluconolactonase', subsystem='Pentose Phosphate'),
            
            # 6-Phosphogluconate dehydrogenase
            cobra.Reaction('GND', name='6-Phosphogluconate dehydrogenase', subsystem='Pentose Phosphate'),
        ]
        ...
        # Set reaction stoichiometry
        # G6PDH2r: glucose-6-phosphate + NADP+ -> 6-phosphoglucono-δ-lactone + NADPH
        ...
        # PGL: 6-phosphoglucono-δ-lactone + H2O -> 6-phosphogluconate
        self.model.reactions.get_by_id('PGL').add_metabolites({
            self.model.metabolites.get_by_id('6pgc_c'): -1,
            self.model.metabolites.get_by_id('h2o_c'): -1,
            self.model.metabolites.get_by_id('6pgc_c'): 1
        })
        
        # GND: 6-phosphogluconate + NADP+ -> ribulose-5-phosphate + NADPH + CO2
        self.model.reactions.get_by_id('GND').add_metabolites({
            self.model.metabolites.get_by_id('6pgc_c'): -1,
            self.model.metabolites.get_by_id('nadp_c'): -1,
            self.model.metabolites.get_by_id('ru5p__D_c'): 1,
            self.model.metabolites.get_by_id('nadph_c'): 1,
            self.model.metabolites.get_by_id('co2_e'): 1
        })
    ...
    def add_glyoxylate_cycle_reactions(self):
        ...
        reactions = [
            # Isocitrate lyase
            cobra.Reaction('ICL', name='Isocitrate lyase', subsystem='Glyoxylate Cycle'),
            
            # Malate synthase
            cobra.Reaction('MALS', name='Malate synthase', subsystem='Glyoxylate Cycle'),
        ]
        ...
        # MALS: glyoxylate + acetyl-CoA -> malate + CoA
    ...
    def add_lactose_metabolism_reactions(self):
        """Add lactose metabolism reactions."""
        reactions = [
            # Lactose uptake
            cobra.Reaction('EX_lac__D_e', name='Lactose exchange', subsystem='Exchange'),
            
            # Lactose permease
            cobra.Reaction('LACY', name='Lactose permease', subsystem='Lactose Metabolism'),
            
            # β-Galactosidase
            cobra.Reaction('LACZ', name='β-Galactosidase', subsystem='Lactose Metabolism'),
        ]
    ...
    def add_exchange_reactions(self):
        ...
        reactions = [
            # Acetate exchange
            cobra.Reaction('EX_ac_e', name='Acetate exchange', subsystem='Exchange'),
            
            # Oxygen exchange
            cobra.Reaction('EX_o2_e', name='Oxygen exchange', subsystem='Exchange'),
            
            # CO2 exchange
            cobra.Reaction('EX_co2_e', name='CO2 exchange', subsystem='Exchange'),
            
            # Ammonium exchange
            cobra.Reaction('EX_nh4_e', name='Ammonium exchange', subsystem='Exchange'),
            
            # Phosphate exchange
            cobra.Reaction('EX_pi_e', name='Phosphate exchange', subsystem='Exchange'),
        ]
        ...
        self.model.reactions.get_by_id('EX_nh4_e').add_metabolites({
            self.model.metabolites.get_by_id('nh4_e'): -1
        })
        
        self.model.reactions.get_by_id('EX_pi_e').add_metabolites({
            self.model.metabolites.get_by_id('pi_e'): -1
        })
    
    def add_biomass_reaction(self):
        """Add biomass reaction."""
        biomass_reaction = cobra.Reaction('BIOMASS', name='Biomass', subsystem='Biomass')
        self.model.add_reactions([biomass_reaction])
        
        # Simplified biomass composition
    ...
    def setup_medium_conditions(self, condition):
        """Setup medium conditions for the custom model."""
        ...
        if condition == 'glucose_minimal':
            self.model.reactions.get_by_id('EX_glc__D_e').bounds = (-8.5, 0)  # Experimental uptake
            self.model.reactions.get_by_id('EX_ac_e').bounds = (0, 0)
            self.model.reactions.get_by_id('EX_lac__D_e').bounds = (0, 0)
            self.model.reactions.get_by_id('EX_o2_e').bounds = (-15.2, 0)  # Experimental uptake
        elif condition == 'acetate_minimal':
            self.model.reactions.get_by_id('EX_glc__D_e').bounds = (0, 0)
            self.model.reactions.get_by_id('EX_ac_e').bounds = (-3.6, 0)  # Experimental uptake
            self.model.reactions.get_by_id('EX_lac__D_e').bounds = (0, 0)
            self.model.reactions.get_by_id('EX_o2_e').bounds = (-8.1, 0)  # Experimental uptake
        elif condition == 'lactose_minimal':
            self.model.reactions.get_by_id('EX_glc__D_e').bounds = (0, 0)
            self.model.reactions.get_by_id('EX_ac_e').bounds = (0, 0)
            self.model.reactions.get_by_id('EX_lac__D_e').bounds = (-0.7, 0)  # Experimental uptake
            self.model.reactions.get_by_id('EX_o2_e').bounds = (-6.3, 0)  # Experimental uptake
        
        # Allow essential nutrients
        self.model.reactions.get_by_id('EX_nh4_e').bounds = (-1000, 0)
        self.model.reactions.get_by_id('EX_pi_e').bounds = (-1000, 0)
        self.model.reactions.get_by_id('EX_co2_e').bounds = (0, 1000)
        
        print(f"{condition} medium setup complete")
    
    def run_custom_analysis(self):
        """Run analysis with the custom model."""
        print("\n" + "=" * 70)
        print("RUNNING CUSTOM MODEL ANALYSIS")
        print("=" * 70)
    ...
    def create_custom_visualizations(self):
        """Create visualizations for custom model results."""
        print("\nCreating custom model visualizations...")
        ...
        for i, (condition, results) in enumerate(self.results.items()):
            ...
            # Get experimental and predicted fluxes
            exp_data = self.c13_data[condition]
            exp_fluxes = []
            pred_fluxes = []
            reaction_names = []
            ...
            for rxn_id, exp_flux in exp_data['fluxes'].items():
                if rxn_id in results['fluxes'] and abs(exp_flux) > 1e-6:
                    exp_fluxes.append(exp_flux)
                    pred_fluxes.append(results['fluxes'][rxn_id])
                    reaction_names.append(rxn_id)
            
            if len(exp_fluxes) >= 3:
                # Scatter plot
                axes[row, col].scatter(exp_fluxes, pred_fluxes, alpha=0.7, s=50)
                
                # Add 1:1 line
                min_val = min(min(exp_fluxes), min(pred_fluxes))
                max_val = max(max(exp_fluxes), max(pred_fluxes))
                axes[row, col].plot([min_val, max_val], [min_val, max_val], 'r--', alpha=0.8, label='1:1 line')
                
                axes[row, col].set_xlabel('Experimental Flux (mmol/gDW/h)')
                axes[row, col].set_ylabel('Predicted Flux (mmol/gDW/h)')
                axes[row, col].set_title(f'{condition.replace("_", " ").title()}\nCorrelation: {results["correlation"]:.3f}, MAPE: {results["mape"]:.1f}%')
                axes[row, col].legend()
                axes[row, col].grid(True, alpha=0.3)
        
        plt.tight_layout()
        plt.savefig('results/custom_model_results.png', dpi=300, bbox_inches='tight')
        plt.close()
        
        print("Custom model visualizations saved to results/custom_model_results.png")
    
    def save_custom_results(self):
        """Save custom model results."""
        print("\nSaving custom model results...")
        
        # Save results
        with open('results/custom_model_results.json', 'w') as f:
            json.dump(self.results, f, indent=2, default=str)
        print("Custom model results saved to results/custom_model_results.json")
    
    def generate_custom_report(self):
        """Generate custom model report."""
        print("\n" + "=" * 70)
        ...
        print("4. ✅ Complete glycolysis and TCA cycle")
        print("5. ✅ Pentose phosphate pathway")
        print("6. ✅ Experimental flux constraints")
        ...
        if avg_growth_error < 10 and avg_correlation > 0.9 and avg_mape < 50:
            print("  ✅ EXCELLENT: Custom model performs exceptionally well")
        elif avg_growth_error < 20 and avg_correlation > 0.8 and avg_mape < 100:
            print("  ✅ GOOD: Custom model shows significant improvement")
        elif avg_growth_error < 40 and avg_correlation > 0.7 and avg_mape < 150:
            print("  ⚠️  FAIR: Custom model shows moderate improvement")
        else:
            print("  ❌ POOR: Custom model needs further refinement")

def main():
    """Main function to run custom model analysis."""
    custom_model = CustomMetabolicModel()
    
    # Build custom model
    if not custom_model.build_custom_model():
        print("❌ Failed to build custom model!")
        return
    
    # Run analysis
    if not custom_model.run_custom_analysis():
        print("❌ Failed to run custom analysis!")
        return
    
    # Create visualizations and save results
    custom_model.create_custom_visualizations()
    custom_model.save_custom_results()
    custom_model.generate_custom_report()
    
    print("\n" + "=" * 70)
    print("CUSTOM MODEL ANALYSIS COMPLETE!")
    print("=" * 70)
    print("✅ Built custom metabolic model for experimental conditions")
