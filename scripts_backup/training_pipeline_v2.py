#!/usr/bin/env python3
"""
Training Pipeline V2 for Metabolic Network Embedding

This module implements a robust training pipeline using the new data handling system.
Features:
- Integration with robust data handling
...
from scripts.data_handling import (
    MetabolicGraph, MetabolicTargets, MetabolicDataset, DataPreprocessor,
    GraphBatch, collate_metabolic_data, create_synthetic_data
)
...
class TrainingPipelineV2:
    """
    Robust training pipeline using the new data handling system.
    """
    
    def __init__(self, 
                 model_config: Dict[str, Any],
                 output_dir: str = "results/metabolic_network/training_v2",
                 device: str = "auto"):
        ...
        logger.info(f"TrainingPipelineV2 initialized on device: {self.device}")
    ...
    def prepare_data(self, 
        ...
        """Prepare data loaders using the new data handling system."""
        ...
        # Create dataset
        dataset = MetabolicDataset(processed_graphs, processed_targets, condition_labels)
        ...
        train_loader = DataLoader(
            train_dataset, batch_size=batch_size, shuffle=shuffle, collate_fn=collate_metabolic_data
        )
        val_loader = DataLoader(
            val_dataset, batch_size=batch_size, shuffle=False, collate_fn=collate_metabolic_data
        )
        test_loader = DataLoader(
            test_dataset, batch_size=batch_size, shuffle=False, collate_fn=collate_metabolic_data
        )
    ...
    def _train_epoch(self, 
        ...
        for batch_idx, batch in enumerate(train_loader):
            try:
                # Move data to device
                graph_batch = batch['graph_batch'].to_device(self.device)
                targets = batch['targets'].to_device(self.device)
                
                # Forward pass
                optimizer.zero_grad()
                predictions = model(graph_batch.node_features, graph_batch.edge_indices)
...
def test_training_pipeline_v2():
    """Test the new training pipeline thoroughly."""
    logger.info("Testing TrainingPipelineV2...")
    ...
    pipeline = TrainingPipelineV2(model_config)
    
    # Create synthetic data using the new data handling system
    graphs, targets = create_synthetic_data(num_samples=100)
    ...
    logger.info("🎉 TrainingPipelineV2 test completed successfully!")

if __name__ == "__main__":
    test_training_pipeline_v2() 